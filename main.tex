...
Because measured range is always non-negative, the Range FFT can be read directly bin-by-bin (no reinterpretation needed). But Doppler velocity (and, in the next section, $\sin\theta$) can be negative, so Doppler and Angle FFT outputs must be reinterpreted (\lstinline{fftshift}-ed) using the formula above to recover signed velocity/angle.
\end{remark}
...
For an array with element positions $p_0, p_1, \dots, p_{N_a-1}$ along a 1-D array axis, the array manifold vector is
...
\end{definition}
Steering (conjugate-matching) the array response to angle $\theta$ coherently combines the signal from that direction. Sweeping $\theta$ and evaluating $|y(t;\theta)|^2$ recovers the angular spectrum for \emph{any} array geometry, reducing to the efficient DFT approach below in the uniform-spacing case.

\begin{remark}[Non-Uniform Arrays]
Grating-lobe and resolution behavior depend on the specific element positions $p_m$, not just the aperture $D = \max_m p_m - \min_m p_m$. TDM-MIMO virtual arrays exploit this: antennas are deliberately placed non-uniformly to extend the aperture (finer angular resolution) while keeping gaps small enough to suppress grating lobes, something a uniform $\lambda/2$ array of the same element count cannot do.
\end{remark}
